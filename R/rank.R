...
plot_rank_correlation <- function(correlation_file, order=TRUE) {
    ...
    p <- (ggplot(correlations, aes(x=V1, y=V3, color=factor(V2))) +
          ...
          scale_y_continuous(name='Hold-out Pearson correlation') +
          scale_color_brewer(palette="Paired", name="Cohort") +
          theme_nature +
